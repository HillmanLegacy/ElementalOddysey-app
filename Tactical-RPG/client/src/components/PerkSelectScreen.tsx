...
import type { PlayerCharacter, Perk } from "@shared/schema";
...
export default function PerkSelectScreen({ player, onSelect }: PerkSelectScreenProps) {
  const availablePerks = PERKS.filter(
    p => !player.perks.includes(p.id) && (p.element === player.element || p.tier <= 1)
  ).slice(0, 6);

  return (
    <div className="relative w-full h-screen overflow-hidden bg-gradient-to-b from-[#0a0a1a] via-[#1a0a2e] to-[#0a0a1a]">
      <ParticleCanvas
        colors={[ELEMENT_COLORS[player.element], COLOR_MAP[player.energyColor], "#a855f7"]}
        count={50}
        speed={0.8}
        style="swirl"
      />
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full px-4">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-purple-400/60 mt-1">Select a new ability for your journey</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg">
          {availablePerks.map(perk => {
            const isElementMatch = perk.element === player.element;
            return (
              <Card
                key={perk.id}
