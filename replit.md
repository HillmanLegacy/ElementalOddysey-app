...
-   **Spell/Magic System**: Level-based spell unlocks unique to each element. Additional spells can be learned from the Shaman's Lair. **Storm Release** (Wind, level 6) is Mifune's unique special-attack spell using `animation: "samuraiWindMagic"` — plays the SPECIAL ATTACK sprite, then launches wind blade VFX at the target.
