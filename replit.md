...
-   **Animation System**:
    -   **Battle Transition**: Cinematic canvas-based transition from overworld to battle (`BattleTransition.tsx`). Originates from player's position with element-colored effects. Phases: ground pulse → ripple → edge vignette → energy fractures (cracking glass) → expanding light → shard fragments spiraling → flash eruption → darkness → battle materializes. Duration: 2.8s. Intercepts `onNodeSelect` in App.tsx to play before `startBattle`.
